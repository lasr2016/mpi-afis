...
//	MCC a2;
    ...
    char str[512], str2[512], str3[512], str_copy_1[512], str_copy_2[512], *token;
    struct timeval t1, t2;
	Elem ***mat_res;
	Elem *heap;
    int id_dir_ini, id_dir_fin, id_dir, largo_ini, largo_fin;
...
#pragma omp parallel private(I, i, h_temp, n_elemH) shared(Queries_MCC, BD_MCC, mat_res, pf)
{
	...
	for (I=0; I < NUM_ELEM_Q; I++)
	{
		 if (tid == 0)
		{
			fprintf(pf, "T[%d] :: Procesando query %d\n", tid, I);
			fflush(pf);
		} 
        ...
        if (FRR)
        {
            ...
            for (i=0; i < TOPK; i++)
            {
                ...
                if (i+1 == TOPK)
                {
                    ...
                    sprintf(str, "cp ~/nas1-HuellasDB72M/");

                    id_dir = (int)(arr_id_Q[I] / 50000);
                    ...
                    //printf("~/nas1-HuellasDB72M/0000050001-0000100000/Huellas-72M_100000_1.wsq");
